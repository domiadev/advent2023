...
class Schematic:
    def __init__(self, input: str):
        # addressible by [y][x]
        self.contents = input.splitlines()
        self.height = len(self.contents)
        self.width = len(self.contents[0])

    def get(self, p: Point2d) -> str:
        return self.contents[p.y][p.x]
    
    # [(number, start, end)]
    def line_nums(self, y: int) -> list[tuple[int, Point2d, Point2d]]:
        return [
            ( int(m.group(1)), Point2d(m.start(0), y), Point2d(m.end(0), y) )
            for m in re.finditer(r'(\d+)', self.contents[y])
        ]
    
    # [(number, start, end)]
    def numbers(self) -> list[tuple[int, Point2d, Point2d]]:
        return [
            ( number, start, end )
            for y in range(0, self.height)
            for number, start, end in self.line_nums(y)
        ]
    
    # [symbol]
    def symbols(self, start: Point2d, end: Point2d) -> list[str]:
        ...
        bottom_right = Point2d(
            min(self.width - 1, end.x + 1),
            min(self.height - 1, end.y + 1),
        )
        return [
            self.get(Point2d(x, y))
                for y in range(upper_left.y, bottom_right.y+1)
                for x in range(upper_left.x, bottom_right.x)
            if (not ((y == start.y) and (start.x <= x < end.x)))
                and self.get(Point2d(x,y)) != '.'
        ]
    
    # [(number, symbols)]
    def numbers_symbols(self) -> list[tuple[int, list[str]]]:
        """
            the numbers and the symbols around them
        """
        return [
            (num, self.symbols(startpt, endpt))
            for num, startpt, endpt in self.numbers()
        ]
    ...
    def parts(self) -> list[int]:
        return [
            part
            for part, symbols in self.numbers_symbols()
            if len(symbols) > 0
        ]
    
    def numbers_surrounding(self, point: Point2d) -> list[int]:
        assert self.get(point) == '*'
        ...
        return {
            ...
            if y == start.y and start.x <= x < end.x
        }
    ...
    def gear_ratios(self) -> list[int]:
        return [
            ...
            if (p := Point2d(x,y))
                and (gear := self.get(p)) and gear == '*'
                and (ratios := list(self.numbers_surrounding(p))) and len(ratios) == 2
        ]
    
def part1(input: str) -> int:
    # the sum of its parts
    return sum(Schematic(input).parts())

def part2(input: str) -> int:
    return sum(Schematic(input).gear_ratios())
        
def main():
    print(f"=== Part 1 ===")
    print(f"example: {part1(ex1)}")
    assert 4361 == part1(ex1)
    print(f"actual:  {part1(input)}")
    assert 550064 == part1(input)

    print(f"=== Part 2 ===")
    print(f"example: {part2(ex1)}")
    assert 467835 == part2(ex1)
    print(f"actual:  {part2(input)}")
    assert 85010461 == part2(input)
