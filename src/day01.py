...
def calibration(line: str):
    ...
    if len(matches) < 1:
        raise Exception(f"line did not match regexp: {line}")
    return int(matches[0]) * 10 + int(matches[-1])

def part1(block: str):
    return sum(
        [calibration(line) for line in block.splitlines()]
    )
...
def to_int(text: str):
    ...
    }.get(text, False) or int(text)

def calibration2(line: str):
    iter = re.finditer(r'(?=(\d|one|two|three|four|five|six|seven|eight|nine))', line)
    matches = [match.group(1) for match in iter]
    if len(matches) < 1:
        raise Exception(f"line did not match regexp: {line}")
    return to_int(matches[0]) * 10 + to_int(matches[-1])
# need to find all OVERLAPPING matches too! tricksy.
assert 88 == calibration2("eightjzqzhrllg1oneightfck")

def part2(block: str):
    return sum(
        [calibration2(line) for line in block.splitlines()]
    )

def main():
    print(f"=== Part 1 ===")
    print(f"example: {part1(ex1)}")
    assert 142 == part1(ex1)
    print(f"actual:  {part1(input)}")
    assert 53080 == part1(input)

    print(f"=== Part 2 ===")
    print(f"example: {part2(ex2)}")
    assert 281 == part2(ex2)
    print(f"actual:  {part2(input)}")
    assert 53268 == part2(input)
...
input = '''eight9fhstbssrplmdlncmmqqnklb39ninejz
three656
