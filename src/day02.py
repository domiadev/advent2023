...
def parse_game(line: str) -> tuple[int, list[dict[str, int]]]:
    parsed = re.match(r'^Game (\d+): (.+)$', line)
    if not parsed:
        raise ValueError(f"Could not parse line {line}")
    id, pulls = parsed.groups()
    return int(id), [parse_pull(pull) for pull in pulls.split('; ')]

def parse_pull(pull: str) -> dict[str, int]:
    counts = re.findall(r'(\d+) (red|green|blue)', pull)
    return { color: int(count) for [count, color] in counts }

def valid_game_id(line: str) -> int:
    '''
    returns the game id if the game is possible, 0 otherwise
    '''
    id, pulls = parse_game(line)
...
def is_pull_possible(pull: dict[str, int]) -> bool:
    return all(
        possibility is True for possibility in 
            [int(count) <= limits.get(color) for color, count in pull.items()]
    )

def part1(input: str) -> int:
    return sum(
        valid_game_id(line) for line in input.splitlines()
    )

def game_power(line: str) -> int:
    _, pulls = parse_game(line)
    max_ball_counts = reduce(
        lambda x, y: { key: max(x.get(key, 0), y.get(key, 0)) for key in set(x) | set(y) },
        pulls,
        {"red": 0, "green": 0, "blue": 0}
    )
    return prod(max_ball_counts.values())

def part2(input: str) -> int:
    return sum(
        game_power(line) for line in input.splitlines()
    )
...
def main():
    ...
    assert 2683 == part1(input)

    print(f"=== Part 2 ===")
    print(f"example: {part2(ex1)}")
    assert 2286 == part2(ex1)
    print(f"actual:  {part2(input)}")
    assert 49710 == part2(input)
